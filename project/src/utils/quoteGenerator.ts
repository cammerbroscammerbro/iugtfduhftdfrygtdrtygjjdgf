...
const QuoteDisplay: React.FC<QuoteDisplayProps> = ({ quote, bgType }) => {
	...
	const selectedBg = POSTER_IMAGES.find(bg => bg.value === bgType) || POSTER_IMAGES[0];
	...
	return (
		<div className="w-full max-w-md mx-auto mb-8">
			<div
				...
				style={selectedBg.value === 'solid-black' ? { background: '#000' } : {}}
			>
				{/* Only show image if not solid black */}
				{selectedBg.value !== 'solid-black' && (
					<div
						className={`absolute inset-0 bg-[url('${selectedBg.url}')] opacity-40 bg-cover bg-center mix-blend-overlay`}
					/>
				)}
				<div className="absolute inset-0 bg-black/60" />
				<div className="text-center relative z-10">
					<div className="w-16 h-1 bg-white/30 mx-auto mb-8" />
					<p className="quote-text">{text}</p>
					{emoji && <p className="quote-emoji">{emoji}</p>}
					<div className="w-16 h-1 bg-white/30 mx-auto mt-8" />
				</div>
			</div>
		</div>
	);
